...
int main ()
{
  ...
  */
  ...
  file_2 << setw( 2 ) << 60 << "\t" << fixed << setw( 40 ) << setprecision(std::numeric_limits<long double>::digits10 + 1) << V_2[60]  << endl;
  file_2 << setw( 2 ) << 59 << "\t" << fixed << setw( 40 ) << setprecision(std::numeric_limits<long double>::digits10 + 1) << V_2[59]  << endl;

  for (int n = 60; n >= 2; n--) {
    V_2[n-2] = M_PI*(1 - M_PI*V_2[n])/(n*(n-1));
    file_2 << setw( 2 ) << n-2 << "\t"  << fixed << setw( 40 ) << setprecision(std::numeric_limits<long double>::digits10 + 1) << V_2[n-2]  << endl;
  }
  ...
  for (int n = 0; n <= 40; n++) {
    ...
    file_3 << setw( 2 ) << n << "\t"  << fixed << setw( 40 ) << setprecision(std::numeric_limits<long double>::digits10 + 1) << V_3[n]  << endl;
  }
